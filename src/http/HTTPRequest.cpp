...
HTTPRequest::HTTPRequest(std::vector<ConfigServer> &servers)
    : HTTPMessage(), multipartForm(NULL), m_isMultipartForm(false),
      m_Error(ERR_NONE), m_TransferEncoding(DEFAULT), m_ConfigServers(servers),
      m_ConfigServer(NULL) {}

const ConfigServer *HTTPRequest::getServer() const { return m_ConfigServer; }
...
void HTTPRequest::_checkMultipart() {
  ...
  HTTPRequest::header_map_t mediaTypes =
      parseHeaderDirectives(contentType, pos);
...
void HTTPRequest::onHeadersParsed() {
  ...
  m_ConfigServer =
      &(Config::getServerByName(m_ConfigServers, m_Headers["host"]));
  _checkMultipart();
}

bool HTTPRequest::isMultipartForm() const { return m_isMultipartForm; }
...
bool HTTPRequest::_validateHeaders() {
  ...
  if (it != m_Headers.end() &&
      isChunked) // has both content-length and chunked, should reject
  {
...
void HTTPRequest::setMethod(const char *method_cstr) {
  ...
  /*
          TODO: check if method in allowed methods
                    otherwise set parseState to PARSE_ERROR ? and status code to
     501 not implemented
  */
  if (method == "GET")
    m_Method = GET;
...
bool HTTPRequest::validUri() { return m_Uri[0] == '/'; }
...
void HTTPRequest::appendUri(const char *buff, size_t start, size_t len) {
  // std::cout << "APPENDING TO PATH: start: " << start << " len: " << len <<
  // std::endl;
...
const std::string &HTTPRequest::getUri() const { return m_Uri; }
