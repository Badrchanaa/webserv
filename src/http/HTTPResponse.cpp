...
HTTPResponse::pollState HTTPResponse::getPollState() const { return m_PollState; }
...
void HTTPResponse::setupCgiEnv(std::string &ScriptFileName)
{
    ...
    for (std::vector<std::string>::iterator it = this->cgi_env.begin(); it != this->cgi_env.end();
         ++it)
    {
...
void HTTPResponse::_initCgi(const std::string path, const CGIHandler &cgihandler,
                            const ConfigServer *configServer)
{
    (void)path;
    (void)configServer;
...
void HTTPResponse::_sendHeaders()
{
    ...
    std::cout << "headers size: " << headersString.length() << " cursorPos:" << m_CursorPos
              << std::endl;
...
void HTTPResponse::_processHeaders()
{
    ...
    for (HTTPRequest::header_map_t::iterator it = m_Headers.begin(); it != m_Headers.end(); it++)
    {
...
void HTTPResponse::_debugBody()
{
    ...
    responseStream << "request query: '" << m_Request->getQuery() << "'" << std::endl;
...
void HTTPResponse::_processResource()
{
	...
	for (std::vector<std::string>::const_iterator it = indexes.begin(); it != indexes.end();
	     it++)
	{
	    std::string index = *it;
...
{
    char buff[READ_BUFFER_SIZE];
    if (m_Request->getMethod() == HEAD || (m_Body.getSize() == 0 && m_SocketBuffer.size() == 0))
    {
	m_State = DONE;
	return;
    }
