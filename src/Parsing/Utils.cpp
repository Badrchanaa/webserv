...
bool Config::is_commented(std::string &line) {
  size_t i = 0;
...
void Config::split_key_value(const std::string &line, std::string &key,
                     std::string &value) {
...
int Config::leading_spaces(const std::string &line) {
  int count = 0;
...
bool Config::is_list_item(const std::string &line) {
  std::string trimmed = this->trim(line);
  return trimmed.size() > 1 && trimmed[0] == '-' && trimmed[1] == ' ';
}

std::string Config::get_list_item(const std::string &line) {
  return this->trim(line.substr(2)); // after "- "
}

/* ++ Convert bitmask back to human-readable string ++ */
std::string Config::method_bit_to_string(unsigned int mask) {
  std::string result;
