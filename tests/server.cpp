...
struct tnode *add_to_tree(struct tnode *p, char *flight_no, bool departure,
                          time_t flight_time);
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    for (int fd = 0; fd < (nfds + 1); fd++) {
      if ((pollfds + fd)->fd <= 0) // file desc == 0 is not expected, as these
                                   // are socket fds and not stdin
        continue;

      if (((pollfds + fd)->revents & POLLIN) ==
          POLLIN) {                           // fd is ready for reading
        if ((pollfds + fd)->fd == listener) { // request for new connection
          ...
          if (numfds == maxfds) { // create space
            if ((pollfds = realloc(pollfds, (maxfds + NUM_FDS) *
        ...
        } else // data from an existing connection, receive it
        {
          ...
          ssize_t numbytes = recv((pollfds + fd)->fd, &recv_message,
          ...
          if (numbytes == -1)
            ...
            if (close((pollfds + fd)->fd) == -1)
              error("close");
            (pollfds + fd)->fd *=
                -1; // make it negative so that it is ignored in future
          } else {
            // data from client
            bool valid;
